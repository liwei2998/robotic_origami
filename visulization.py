...
def VisualState(state,adj_facets,count=0,w=350,h=350):
    '''
    ...
    for i in range(column):
        ...
        for j in src:
            ...
            for state in src_tmp:
                if hp.determineWeight(state_dict[j],state_dict[state]) > 100000000:
                    continue
                elif hp.determineWeight(state_dict[j],state_dict[state]) <= 100000000:
