...
def determineWeight(state1,state2):
    ...
    if len(state2['stack']) - len(state1['stack']) > 1:
        ...
        method = 'flexflip'
    # kind 1: flexflip, valley, non-reflect
    if method == 'flexflip' and state2['fold'] == 'valley' and state2['reflec'] == 0:
        weight = 8
    # 1: if valley of mountain
    if state2['fold'] == 'mountain':
        weight = weight - 1.0
    # 2: if reflect
    elif state2['reflec'] == 1:
        weight = weight - 1.0
    # 3:if flexflip or scooping
    elif len(state2['stack']) - len(state1['stack']) > 1:
        weight = weight - 1.0
...
def drawGraph(state_dict,state_graph_culled,path,weight=0,pos=0):
    ...
    for state in state_graph_culled.keys():
        ...
        for kid in state_graph_culled[state]:
            ...
            G.add_edge(state,kid,weight=3)
    nx.draw_networkx(G,pos=pos,node_size=3000,node_color='#00CED1',node_shape='s',alpha=0.5,width=3)
