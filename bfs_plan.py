...
state1 = {"stack":stack1,"polygen":polygen1,"facet_crease":facets1,
          ...
          "adjacent_facets":adjacent_facets,"fold":0}
...
def bfs(state_graph, src, tgt_stack):
    ...
    while queue:
        ...
        children_states = osg.generateNextLayerStates(state_node,state1["adjacent_facets"])
...
def visualSteps(state_dict,path):
    ...
    # img=vl.drawPolygon(polygen2,stack2,canvas,rot_mat,0,count)
    # imgs.append(img)
    for i in range(len(path)):
        canvas = vl.init_canvas(w,h)
        rot_mat = vl.rotationFromImg(w,h,0)
