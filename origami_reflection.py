...
def ifLineIntersect(line1,line2):
    #return if two linear lines has intersection
    is_meet = npi.intersection(line1,line2)
    if len(is_meet)>0:
        return True
    x11 = line1[0][0]
    ...
    x21 = line2[0][0]
    x22 = line2[1][0]
    if max(x11,x12) < max(x21,x22) and max(x11,x12) > min(x21,x22):
        return True
    if max(x11,x12) > max(x21,x22) and max(x11,x12) < min(x21,x22):
        return True
    return False
...
def findFacetwithSameEdge(crease_edge,stack,edge,facet):
    ...
    facets = []
    # print "edge",edge
    for i in crease_edge[facet]:
        if ifLineColinear(i,edge) == 1:
            edge = i
            break
    # print 'edge2',edge
